...
exports.analyzeImageWithGeminiServerSide = void 0;
...
const analyzeImageWithGeminiServerSide = async (apiKey, base64ImageData, imageMimeType) => {
    ...
    try {
        ...
        if (jsonStr === undefined || jsonStr === null) {
            ...
            return { analysis: null, candidates: response.candidates, error: noticeMsg ? undefined : errorMessage, notice: noticeMsg };
        }
        jsonStr = jsonStr.trim();
        if (!jsonStr) {
            logger.warn("Gemini analysis returned empty string content.");
            return { analysis: null, candidates: response.candidates, notice: "AI analysis resulted in empty content." };
        }
        ...
        try {
            ...
            if (parsedData && typeof parsedData.productName === 'string' && typeof parsedData.description === 'string') {
                ...
                return { analysis: parsedData, candidates: response.candidates };
            }
            else {
                logger.warn("Parsed JSON from Gemini analysis is not in the expected format:", parsedData);
                return { analysis: null, candidates: response.candidates, error: `AI's response was not in the expected JSON format.` };
            }
        }
        catch (e) {
            logger.error("Failed to parse JSON response from Gemini analysis:", e, "Raw response snippet:", jsonStr.substring(0, 200));
            return { analysis: null, candidates: response.candidates, error: `Could not parse AI's JSON response. The AI might have returned text that is not valid JSON.` };
        }
    }
    catch (error) {
...
exports.analyzeImageWithGeminiServerSide = analyzeImageWithGeminiServerSide;
